...
chdir("..");
$originalDirectory = getcwd()."/mapaPunto/";
$editDirectory = getcwd()."/edit/";
$activeDirectory=$editDirectory;
...
$cadExiste="original: ".$resorig." --- activo: ".$resact." --- editar: ".$resedit;

/*
if(empty($imageName) ||
	!file_exists($originalDirectory.$imageName) ||
	!file_exists($activeDirectory.$imageName) ||
	!file_exists($editDirectory.$imageName)) { echo "{imageFound:false,directorio:'".$cadExiste."'}"; exit; }
*/	
if( (empty($imageName)) || (!file_exists($originalDirectory.$imageName)) ) 
{ 
	echo "{imageFound:false,directorio:'".$cadExiste."'}";
	exit; 
}	
...
list($w, $h) = getimagesize($originalDirectory.$imageName);
